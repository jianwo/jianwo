...
module.exports = mailController = app.controller 'mail', ->
  ...
  @action 'dm', (req, res, callback) ->
    ...
    .then ([user, sender, team, messages]) ->
      ...
      email.team = team
      email.num = '20+'
      email.subject = "[席地] #{email.sender?.name}给你发送了新消息"
  ...
  @action 'rm', (req, res, callback) ->
    ...
    .then ([user, room, messages]) ->
      ...
      email.user = user
      email.num = '20+'
      email.subject = "[席地] 来自#{room.topic}的新消息"
  ...
  @action 'gm', (req, res, callback) ->
    ...
    .then ([user, room, messages]) ->
      ...
      email.user = user
      email.num = '20+'
      email.subject = "[席地] 话题 “#{room.topic}” 中的聊天记录"
  ...
  @action 'invite', (req, res, callback) ->
    ...
    .then ([sender, team]) ->
      ...
      email.team = team
      email.label = "团队"
      email.subject = "[席地] #{sender.name}邀请你加入#{email.label}"
